...
def run(args, index=None):
    ...
    out_dir = os.path.join(scene_dir, 'flow_exhaustive')
    ...
    start_flow_file = os.path.join(
        scene_dir, 'flow_exhaustive', '{}_{}.npy'.format(imgname_i, imgname_i_plus_1))
    start_flow = np.load(start_flow_file)
    start_flow = torch.from_numpy(start_flow).float()[
        None].cuda()  # [b, h, w, 2]

    start_mask_file = start_flow_file.replace(
        'flow_exhaustive', 'raft_masks').replace('.npy', '.png')
    start_cycle_mask = imageio.imread(start_mask_file)[..., 0] > 0

    feature_i = features[i].permute(2, 0, 1)[None]
    feature_i = F.interpolate(
        feature_i, size=start_flow.shape[1:3], mode='bilinear')
    ...
    for j in range(i + 1, num_imgs):
        ...
        direct_flow_file = os.path.join(
            scene_dir, 'flow_exhaustive', '{}_{}.npy'.format(imgname_i, imgname_j))
        direct_flow = np.load(direct_flow_file)
        direct_flow = torch.from_numpy(direct_flow).float()[
            None].cuda()  # [b, h, w, 2]
        direct_mask_file = direct_flow_file.replace(
            'flow_exhaustive', 'raft_masks').replace('.npy', '.png')
        ...
        curr_coords_normed = normalize_coords(
            curr_coords, h, w)  # [b, h, w, 2]

        feature_j = features[j].permute(2, 0, 1)[None]
        feature_j_sampled = F.grid_sample(
            feature_j, curr_coords_normed, align_corners=True)
        feature_sim = torch.cosine_similarity(
            feature_i, feature_j_sampled, dim=1).squeeze(0).cpu().numpy()
        image_j_sampled = F.grid_sample(
            images[j], curr_coords_normed, align_corners=True).squeeze()
        rgb_sim = torch.norm(
            images[i] - image_j_sampled, dim=1).squeeze(0).cpu().numpy()
        ...
        np.save(os.path.join(out_dir, '{}_{}.npy'.format(
            imgname_i, imgname_j)), accumulated_flow[0].cpu().numpy())
        out_mask = np.concatenate([255 * accumulated_cycle_mask[..., None].astype(np.uint8),
                                   direct_masks[..., 1:]],
                                  axis=-1)
        imageio.imwrite('{}/{}_{}.png'.format(out_mask_dir,
                        imgname_i, imgname_j), out_mask)
        ...
        flow_file = os.path.join(
            scene_dir, 'flow_exhaustive', '{}_{}.npy'.format(imgname_j, imgname_j_plus_1))
        curr_flow = np.load(flow_file)
        curr_flow = torch.from_numpy(curr_flow).float()[
            None].cuda()  # [b, h, w, 2]
        curr_mask_file = flow_file.replace(
            'flow_exhaustive', 'raft_masks').replace('.npy', '.png')
    ...
    for i in range(num_imgs - 1, 0, -1):
        ...
        start_flow_file = os.path.join(
            scene_dir, 'flow_exhaustive', '{}_{}.npy'.format(imgname_i, imgname_i_minus_1))
        start_flow = np.load(start_flow_file)
        start_flow = torch.from_numpy(start_flow).float()[
            None].cuda()  # [b, h, w, 2]
        ...
        for j in range(i - 1, -1, -1):
            ...
            flow_file = os.path.join(
                scene_dir, 'flow_exhaustive', '{}_{}.npy'.format(imgname_j, imgname_j_minus_1))
            curr_flow = np.load(flow_file)
            curr_flow = torch.from_numpy(curr_flow).float()[
                None].cuda()  # [b, h, w, 2]
            curr_mask_file = flow_file.replace(
