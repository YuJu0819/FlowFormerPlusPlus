...
def run_filtering(args, index=None):
    feature_name = 'dino'
    scene_dir = args.data_dir
    slide_len = 10
    print('flitering raft optical flow for {}....'.format(scene_dir))

    img_files = sorted(glob.glob(os.path.join(scene_dir, 'color', '*.jpg')))
    ...
    grid_normed = normalize_coords(
        grid.squeeze().permute(1, 2, 0), h, w)  # [h, w, 2]
    ...
    for j in range(i, i-slide_len-1, -1):
        i = index
        imgname_i = os.path.basename(img_files[i])
        feature_i = features[i].permute(2, 0, 1)[None]
        feature_i_sampled = F.grid_sample(
            feature_i, grid_normed[None], align_corners=True)[0].permute(1, 2, 0)

        frame_interval = abs(i - j)
        imgname_j = os.path.basename(img_files[j])
        flow_f = np.load(os.path.join(
            scene_dir, 'flow_exhaustive', '{}_{}.npy'.format(imgname_i, imgname_j)))
        flow_f = torch.from_numpy(flow_f).float().permute(2, 0, 1)[
            None].cuda()
        flow_b = np.load(os.path.join(
            scene_dir, 'flow_exhaustive', '{}_{}.npy'.format(imgname_j, imgname_i)))
        flow_b = torch.from_numpy(flow_b).float().permute(2, 0, 1)[
            None].cuda()
        # print(i, j)
        coord2 = flow_f + grid
        coord2_normed = normalize_coords(
            coord2.squeeze().permute(1, 2, 0), h, w)  # [h, w, 2]
        flow_21_sampled = F.grid_sample(
            flow_b, coord2_normed[None], align_corners=True)
        ...
        feature_j_sampled = F.grid_sample(
            feature_j, coord2_normed[None], align_corners=True)[0].permute(1, 2, 0)
        feature_sim = torch.cosine_similarity(
            feature_i_sampled, feature_j_sampled, dim=-1)
        ...
        if frame_interval < 3:
            ...
            coord_21_normed = normalize_coords(
                coord_21.squeeze().permute(1, 2, 0), h, w)  # [h, w, 2]
            flow_22 = F.grid_sample(
                flow_f, coord_21_normed[None], align_corners=True)
            fbf_discrepancy = torch.norm(
                (coord_21 + flow_22 - flow_f - grid).squeeze(), dim=0)
            mask_in_range = (coord2_normed.min(
                dim=-1)[0] >= -1) * (coord2_normed.max(dim=-1)[0] <= 1)
            mask_occluded = (fbf_discrepancy < args.cycle_th) * \
                (fb_discrepancy > args.cycle_th * 1.5)
            mask_occluded *= mask_in_range
        else:
            mask_occluded = torch.zeros_like(mask_cycle)

        out_mask = torch.stack(
            [mask_cycle, mask_occluded, torch.zeros_like(mask_cycle)], dim=-1).cpu().numpy()
        imageio.imwrite('{}/{}_{}.png'.format(out_dir, imgname_i,
                        imgname_j), (255 * out_mask.astype(np.uint8)))
    ...
    for i in range(num_imgs):
        ...
        imageio.imwrite(os.path.join(
            count_out_dir, img_name.replace('.jpg', '.png')), count_maps[i])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cycle_th', type=float, default=3.,
                        help='threshold for cycle consistency error')
